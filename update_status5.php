...
if (isset($_POST['admission_id'])) {
    ...
    $sql = "SELECT dept_labs FROM tbl_migration_form WHERE admission_id = '$admission_id'";
    $result = $conn->query($sql);

    if ($result) {
        if ($result->num_rows > 0) {
            $row = $result->fetch_assoc();
            $current_status = $row['dept_labs'];

            // Toggle the status
            $new_status = ($current_status === 'Not Approved') ? 'Approved' : 'Not Approved';

            // Update the status in the database
            $update_sql = "UPDATE tbl_migration_form SET dept_labs = '$new_status', dept_approvedby = '". $_SESSION['admin_name'] ."' WHERE admission_id = '$admission_id'";
