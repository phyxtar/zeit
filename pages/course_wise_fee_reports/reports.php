...
<div class="card">
    <div class="card-header">
        ...
        <form method="POST" action="export_course_yearwise2.php">
            <input type="hidden" name="course_id" value="<?= $course_id ?>" />
            <input type="hidden" name="academic_year" value="<?= $academic_year ?>" />
            <input type="hidden" name="action" value="export_fees_details" />
    ...
    <tbody id="myTable">
        ...
        <tr>
                ...
                <table class="childTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td> <?= $arrayTblFeeUpdate->payment_date ? '' : date('d-M-Y', strtotime($arrayTblFeeUpdate->payment_date)); ?>
                            </td>
            ...
            <!-- <td> <?php
                                    ...
                                    if ($fee_status_data != '') {
                                        $status = $fee_status_data['exam_status'];
                                    } else {
                                        $status = "Not Approve";
                                    }
                ...
                <button id="status<?= $row['admission_id'] ?>"
                    onclick="change_status(<?= $row['admission_id'] ?>,'<?= $status ?>')"
                    class="btn btn-<?= $status == "Approve" ? 'success' : 'danger' ?> btn-sm">
                    <i id="status-fa<?= $row['admission_id'] ?>"
                        class="fas fa-user-<?= $status == "Approve" ? 'check' : 'times' ?> "></i>
                </button>
            </td> -->
...
<div class="row" id="total_fee_hide">
    ...
    <div class="col-2">
        <div class="btn btn-info btn-sm">Total Fine - <?= $total_fine_by_particular_remaning ?></div>
    </div>
    <div class="col-2">
        <div class="btn btn-danger btn-sm">Total Due -
            <?= (int)$total_fee_amount - (int)$total_paid_amount -  (int)$total_rebate_amount ?></div>
    </div>
    <div class="col-2">
        <div class="btn btn-secondary btn-sm"> <strong>Overall Paid -
                <?= (int)$total_rebate_amount + (int)$total_paid_amount  ?> </strong> </div>
    </div>
</div>
<?php
}
