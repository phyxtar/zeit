...
<div class="card">
    <div class="card-header">
        <form method="POST" action="export_course_yearwise1.php">
            <input type="hidden" name="course_id" value="<?= $course_id ?>" />
            <input type="hidden" name="academic_year" value="<?= $academic_year ?>" />
            <input type="hidden" name="action" value="export_fees_details" />
            <button type="submit" class="btn btn-warning float-left"><i class="fa fa-download"></i> Export All</button>
        </form>
        <input class="form-control mr-sm-2 float-right" style="width:30%;" type="search" placeholder="Search"
            id="search-student" onkeyup="searchStudent()" aria-label="Search">
    </div>
</div>
<table id="example1" class="table table-bordered table-striped display" style="width:100%;">
    ...
    <thead>
        <tr>
            ...
            <th width="10%">Examination Status</th>
        </tr>
    </thead>
    <tbody id="myTable">
        <?php
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    $sql_session = "SELECT * FROM `tbl_university_details` WHERE `status` = '" . $visible . "' AND
        `university_details_id` = '" . $row['admission_session'] . "'";
        ...
        <?php
                    // echo "Admission ID: " . $row['admission_id'] . "<br>";
                    // echo "Admission Session: " . $row['admission_session'] . "<br>";
                    // echo "Academic Year: " . $completeSessionOnlyYear;
                    // echo "<hr>";
                    ?>
        <tr class="search-row">
            <td>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <?php
                                    ...
                                    if ($total_remaing_amount_final == 0 && $current_reg_status == 'Approve') {
                                        $current_exam_status = 'Approve';
                                    } else {
                                        $current_exam_status = 'Not Approve';
                                    }

                                    // Update the database with the new exam status
                                    if ($fee_status_data) {
                                        // Update existing record
                                        $update_exam_status = "UPDATE `tbl_fee_status` 
                                                               SET `exam_status` = '" . $current_exam_status . "' 
                                        ...
                                        $insert_exam_status = "INSERT INTO `tbl_fee_status` (`admission_id`, `exam_status`) 
                                                               VALUES ('" . $row['admission_id'] . "', '" . $current_exam_status . "')";
            ...
            <td>
                <?php
                            ...
                            ?>
                <button id="reg_status<?= $row['admission_id'] ?>"
                    onclick="change_status_reg(<?= $row['admission_id'] ?>,'<?= $reg_status ?>')"
                    class="btn btn-<?= $reg_status == "Approve" ? 'success' : 'danger' ?> btn-sm">
                    <i id="reg_status-fa<?= $row['admission_id'] ?>"
                        class="fas fa-user-<?= $reg_status == "Approve" ? 'check' : 'times' ?> "></i>
                </button>
            </td>
            <?php
                        ...
                        $current_status = $fee_status_data ? $fee_status_data['exam_status'] : ($total_remaing_amount_final == 0 ? 'Approve' : 'Not Approve');

                        ?>

            <td>
                <button id="status<?= $row['admission_id'] ?>"
                    onclick="change_status(<?= $row['admission_id'] ?>, '<?= $current_status ?>')"
                    class="btn btn-<?= $current_status == 'Approve' ? 'success' : 'danger' ?> btn-sm">
                    <i id="status-fa<?= $row['admission_id'] ?>"
                        class="fas fa-user-<?= $current_status == 'Approve' ? 'check' : 'times' ?>"></i>
                </button>
            </td>
        </tr>
        <?php
...
<script>
function change_status_reg(student_id, reg_status) {
    const xhttp = new XMLHttpRequest();
    xhttp.onload = function() {
        console.log(this.responseText);
        if (this.responseText == 1) {
            // adding the class show success
            document.getElementById("reg_status" + student_id).classList.add('btn-danger');
            document.getElementById("reg_status-fa" + student_id).classList.add('fa-user-times');

            // removing the class
            document.getElementById("reg_status" + student_id).classList.remove('btn-success');
            document.getElementById("reg_status-fa" + student_id).classList.remove('fa-user-check');


        } else {
            document.getElementById("reg_status" + student_id).classList.add('btn-success');
            document.getElementById("reg_status-fa" + student_id).classList.add('fa-user-check');
            document.getElementById("reg_status" + student_id).classList.remove('btn-danger');
            document.getElementById("reg_status-fa" + student_id).classList.remove('fa-user-times');
        }
    }
    xhttp.open("GET", "pages/course_wise_fee_reports/reg_status.php?id=" + student_id + "&reg_status=" + reg_status,
        true);
    xhttp.send();
}
</script>
...
<script>
function change_status(student_id, current_status) {
    // Determine the new status to toggle
    const new_status = current_status === 'Approve' ? 'Not Approve' : 'Approve';
    const xhttp = new XMLHttpRequest();

    xhttp.onload = function() {
        console.log(this.responseText);
        if (this.responseText == 1) {
            // Update the button appearance based on new status
            const button = document.getElementById("status" + student_id);
            const icon = document.getElementById("status-fa" + student_id);

            // Toggle button classes based on new status
            button.classList.toggle('btn-success', new_status === 'Approve');
            button.classList.toggle('btn-danger', new_status === 'Not Approve');
            icon.classList.toggle('fa-user-check', new_status === 'Approve');
            icon.classList.toggle('fa-user-times', new_status === 'Not Approve');
        }
    };

    // Send the request with manual flag set to 1
    xhttp.open("GET", "pages/course_wise_fee_reports/status.php?id=" + student_id + "&status=" + new_status +
        "&manual=1", true);
    xhttp.send();
}
</script>