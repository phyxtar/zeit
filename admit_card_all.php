...
<body class="body_pop" style="color:#0d1065;">
    ...
    $sql_semester = "SELECT * FROM tbl_allot_semester WHERE semester_id = '$semester_id' AND course_id = '$course_id' AND academic_year = '$academic_year'";
    $result_semester = mysqli_query($con, $sql_semester);
    while ($allot_semester = mysqli_fetch_assoc($result_semester)) {

        $sql = "SELECT * FROM `tbl_admission`
        INNER JOIN `tbl_allot_semester` ON `tbl_admission`.`admission_id` = `tbl_allot_semester`.`admission_id`
        WHERE `tbl_admission`.`admission_id` = '" . $allot_semester['admission_id'] . "'
        AND `tbl_allot_semester`.`allot_id` = '" . $allot_semester['allot_id'] . "'
        AND `tbl_allot_semester`.`course_id` = '" . $course_id . "'";
        $result = mysqli_query($con, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
            $sql_course = "SELECT * FROM `tbl_course` WHERE `status` = '$visible' AND `course_id` = '" . $row["course_id"] . "'";
            $result_course = $con->query($sql_course);
            $row_course = $result_course->fetch_assoc();

            $sql_exam = "SELECT * FROM tbl_examination_form WHERE `admission_id` = '" . $allot_semester['admission_id'] . "' AND `course_id` = '" . $allot_semester['course_id'] . "' AND `academic_year` = '" . $allot_semester['academic_year'] . "' AND `semester_id` = '" . $allot_semester['semester_id'] . "'";
            $result_exam = mysqli_query($con, $sql_exam);
            $row_exam = mysqli_fetch_assoc($result_exam);

            $sql_semester_details = "SELECT * FROM `tbl_semester` WHERE `status` = '$visible' AND `semester_id` = '" . $row["semester_id"] . "'";
            $result_semester_details = $con->query($sql_semester_details);
            $row_semester_details = $result_semester_details->fetch_assoc();
                ...
                <td colspan="3" class="" align="center">&nbsp;&nbsp;<span class="admin">
                        <ul style="margin-top: -120px;font-size: 16px; text-align: center;">
                            <span style="border-bottom: 2px solid;">
                                <?php echo strtoupper($row_semester_details["examname"]) ?>
                                <?php echo strtoupper($row_semester_details["semester"]) ?>
                                <?php echo strtoupper($row_semester_details["examination_month"]) ?>
                            </span>
                        </ul>
                    </span></td>
            </tr>
                                            ...
                                            <tr>
                                                <td height="25" colspan="3" class="tb1_grid">
                                                    &nbsp;&nbsp;<span class="admin"
                                                        ...
                                                        NO:
                                                        <?php echo $row["reg_no"] ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                                        <b style="margin-left: 50px;">COURSE: <?php
                                                                ...
                                                                ?></b>&nbsp;&nbsp;&nbsp;&nbsp;<b
                                                            style="margin-left: 86px;">YEAR OF
                                                            REGISTRATION:
                                                            <?php echo $year; ?></b>
                                                        </b>&nbsp;&nbsp;&nbsp;&nbsp;<b style="margin-left: 86px;">APAAR
                                                            ID:
                                                            <?php echo $row['apaar_id']; ?></b></span></td>
                                            </tr>
                                        </table>
                                        <table
                                            ...
                                            <tr>
                                                ...
                                                <td align="left" class="narmal" width="20%" ;>
                                                    <b><?php echo date('d-M-Y', strtotime($row["admission_dob"])); ?>
                                                    </b>
                                                </td>
                                                <th rowspan="3"
                                    ...
                                    <td>
                                        <table width="100%" class="" style="margin-top: -4px;">
                                            ...
                                            <tbody style="background-size:cover; margin-top:25px;">
                                                <?php
                                                        $sql = "SELECT * FROM `tbl_subjects` WHERE course_id = '" . $row["course_id"] . "' AND semester_id = '" . $row["semester_id"] . "' AND fee_academic_year = '" . $row["academic_year"] . "' AND (specialization_id = '" . $row['specialization_id'] . "' OR specialization_id = '" . $row['specialization_id_dual'] . "' OR COALESCE(specialization_id, '') = '')ORDER BY `subject_id` ASC;";
                                                        $rows = $con->query($sql);
                                    ...
                                    <td>
                                        <table width="100%"
                                            ...
                                            <td class="narmal"><b>REPORTING TIME </b><b style="margin-left: 17px;">:
                                                </b><b><?php echo strtoupper($row_semester_details["exam_reporting_time"]) ?>
                                                </b></br></br><b>TIME OF EXAMINATION </b><b
                                                    style="margin-left: 18px;">:<?php echo strtoupper($lTime = $row_semester_details["time_of_examination"]) ?>
                                                </b>
                                                <b><?php

                                                        // echo date('h:i A', strtotime($lTime));
                                                        if ($row_semester_details["time_of_dispersal"] != "") {
                                                            $mTime = $row_semester_details["time_of_dispersal"];
        ...
        <tr>
            <table>
                <div class="controller">
                    ...
                    <p class="cust"><b>SIGN. OF H.O.D/PRINCIPAL</b></p>
                    <div>
                        <?php
                        ...
                        <tr>
                            <td style="padding: 10px;border: 1px solid;margin-right: 50px;margin-left: 80px;">
                                ...
                                <ol style="font-size:12px;text-align: justify;color:#0d1065;">
                                    ...
                                    <li>Bring your Admit Card and keep it always with you as long as you are in the
                                        Exam-Centre.
                                        You will not to be allowed entry in the exam hall/room without admit card and
                                        student
                                        ID.</li>
                                    <li>Do not bring any unauthorised material, if found, it will be confiscated.</li>
                                    <li>Ensure that you use the washroom before arriving for your exam as you will not
                                        be
                                        permitted to leave during the first hour.</li>
                                    <li>Normally, you are required to answer questions using blue or black ink (Ball
                                        Pen), no
                                        other colour of ink is allowed to write.</li>
                                    <li>Arrive at least 15 minutes before the exam is due to start. As you enter show
                                        your ID
                                        card.</li>
                                    <li>Make sure your mobile phone is switched off and place as advised by the
                                        invigilator.
                                        Bags are not allowed in the examination room.</li>
                                    <li>Remember that talking is strictly prohibited at any time in the exam hall/room.
                                    </li>
                                    <li>Listen carefully to instructions. Students are required to comply with
                                        instruction of
                                        given by invigilators at all times.</li>
                                    <li>You are not allowed to leave the exam rooms in the first hour and last fifteen
                                        minutes.
                                    </li>
                                    <li>If you have any queries or need more papers, raise your hand, the invigilator
                                        will come
                                        to you.</li>
                                    <li>Examinees should clearly write their Roll Number, Registration No., Subject code
                                        and
                                        Paper Name and Date of Examination on the Answer sheet. Such answer sheet,
                                        having no
                                        aforesaid details, will not go for evaluation.</li>
                                    <li>After final submission of Answer sheet, the copy will not be handover to the
                                        examinee
                                        under any circumstances.</li>
                                    <li>Within one hour of question paper distribution, no examinee will be allowed to
                                        submit
                                        the answer sheet.</li>
                                    <li>Once the examinee leaves the examination room, he/she will not be allowed to
                                        re-enter
                                        the examination room.</li>
                                    <li>In case of boycott, repeat examination will not be held under any circumstances.
                                    </li>
                                    <li>Keep your eyes on your own paper. Remember, copying is cheating!</li>
                                    <li>Stop writing immediately when the invigilator asks it is the end of the exam.
                                    </li>
                                    <li>Leave the exam hall/room quickly and quietly. Remember to take all your
                                        belongings with
