...
if (isset($_POST["action"]) && $_POST["action"] == "export_fees_details") {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            foreach ($arrayTblFee as $arrayTblFeeUpdate) {
                ...
                //*************THIS PART WAS GIVING ERROR IN FEE STATUS COLUMN WHEN EXPORTED**************
                // if ((($arrayTblFeeUpdate->fee_remaining) - ($arrayTblFeeUpdate->fee_rebate)) == 0) {
    ...
    //Set Filename
    $fileName = "Course-And-Year-Wise-No-Dues-Fee-Report-" . date('d-m-Y') . ".xlsx";
    //Set BackGround
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B2', "Course And Year Wise No Dues Fee Report - " . $row_course['course_name'] . " (" . $completeSessionOnlyYear . ")");
    //         cellColor('B2', 'FFC107');
    cellColor('B2', 'DC3545');
    ...
    foreach ($dataArray as $completeInfoEach) {
        $hasData = true; // Flag to track if the student has any associated data
