...
if (isset($_POST["action"]) && $_POST["action"] == "export_fees_details") {
    ...
    if ($course_id == "all" && $academic_year == "all" && $gender == "all")
        $sql = "SELECT * FROM `tbl_admission`
                             WHERE `status` = '$visible' && `admission_hostel` = 'yes' && `stud_status` = 1
                             ORDER BY `admission_id` ASC
                             ";
    else if ($course_id == "all" && $academic_year == "all")
        $sql = "SELECT * FROM `tbl_admission`
            WHERE `status` = '$visible' && `admission_gender` = '$gender' && `admission_hostel` = 'yes' && `stud_status` = 1
            ORDER BY `admission_id` ASC
            ";
    else if ($gender == "all")
        $sql = "SELECT * FROM `tbl_admission`
         WHERE `status` = '$visible' && `admission_session` = '$academic_year' && `admission_course_name` = '$course_id'  && `admission_hostel` = 'yes' && `stud_status` = 1
         ORDER BY `admission_id` ASC
         ";
    else
        $sql = "SELECT * FROM `tbl_admission`
         WHERE `status` = '$visible' && `admission_session` = '$academic_year' && `admission_gender` = '$gender' && `admission_course_name` = '$course_id'  && `admission_hostel` = 'yes' && `stud_status` = 1
         ORDER BY `admission_id` ASC
         ";
