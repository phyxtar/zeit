...
if (isset($_POST['admission_id'])) {
    ...
    $sql = "SELECT IT_lab FROM tbl_migration_form WHERE admission_id = '$admission_id'";
    $result = $conn->query($sql);

    if ($result) {
        if ($result->num_rows > 0) {
            $row = $result->fetch_assoc();
            $current_status = $row['IT_lab'];

            // Toggle the status
            $new_status = ($current_status === 'Not Approved') ? 'Approved' : 'Not Approved';

            // Update the status in the database
            $update_sql = "UPDATE tbl_migration_form SET IT_lab = '$new_status', it_approvedby = '". $_SESSION['admin_name'] ."' WHERE admission_id = '$admission_id'";
