...
<head>
    ...
    <title>NETAJI SUBHAS UNIVERSITY | Provisional Certificate </title>
    <!-- Fav Icon -->
...
$sql = "SELECT reg_no, course_id, academic_year FROM `tbl_allot_semester` WHERE `admission_id` = '$admission_id' AND `status` = '$visible'";
$result = $con->query($sql);
$row = $result->fetch_assoc();
$reg_no = $row['reg_no'];
                    ...
                    <div class="row mb-2">
                        ...
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                ...
                                <li class="breadcrumb-item active">Provisional Certificate</li>
                            </ol>
                        </div>
                    </div>
                </div><!-- /.container-fluid -->
                        ...
                        <div class="card">
                            ...
                            <div class="card-body" id="data_table">
                                ...
                                <div class="text-center">
                                    <?php
    $admission_id = $_SESSION['user']['admission_id'];
    $sql_check = "SELECT * FROM `tbl_provisional` WHERE `student_id` = '$admission_id'";
    $result_check = $con->query($sql_check);
    if ($result_check->num_rows == 0) {
    ?>
                                    <p class="text-danger"><b>NOTE:</b> You can request for provisional certificate only
                                    ...
                                    <form id="provisionalForm" method="POST">
                                        ...
                                        <button type="submit" class="btn btn-primary">Request For Provisional
                                            Certificate</button>
                                    </form>
                                    <?php
    } else {
        ...
        if ($row['status'] == 'pending') {
            echo '<p id="successMessage" class="text-success">Your request is successfully submitted. Please wait while we are processing your provisional certificate.</p>';
        } elseif ($row['status'] == 'Approved') {
            echo '<p class="text-success">Your provisional certificate is ready. <a href="download_provisional.php?student_id=' . $_SESSION['user']['admission_id'] . '" class="btn btn-success">Download Certificate</a></p>';
        } else {
    ...
    ?>
                            ...
                            document.getElementById('provisionalForm').addEventListener('submit', function(e) {
                                ...
                                fetch('', {
                                    ...
                                    .then(response => response.text())
                                    .then(data => {
                                        alert('Provisional certificate request submitted successfully.');
                                    })
                                    .catch(error => {
                                        console.error('Error:', error);
                                    });
                            ...
                            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                                ...
                                $sql_provisional = "INSERT INTO `tbl_provisional` (`student_id`, `session`, `course_id`, `date_and_time`, `status`) VALUES ('$student_id', '$session', '$course_id', '$date_and_time', '$status')";
                                if ($con->query($sql_provisional) === TRUE) {
                                    echo "<script>alert('Provisional certificate request submitted successfully.');</script>";
                                } else {
