...
<body class="hold-transition sidebar-mini" oncontextmenu="return false;">
    ...
    <div class="wrapper">
        ...
        <div class="content-wrapper">
            ...
            $allot_attendance = $student_data['attendance'];
            $allot_student = $student_data['admission_id'];


            $sql_sem = "SELECT * FROM `tbl_semester`
            WHERE `semester_id`='$semester_id' ORDER BY `semester_id` DESC ";
            $sem_result = $con->query($sql_sem);
            $sem_row = $sem_result->fetch_assoc();
            $today = date("Y-m-d");
            $attendance = $sem_row['attendance'];
            ...
            } elseif ($allot_attendance < $attendance) {
            ?>


            <div class="container-fluid mt-2">
                <div class="alert alert-warning d-flex align-items-center p-4" role="alert"
                    style="border-radius: 15px; background-color: #fff3cd; border-left: 5px solid #ffc107;">
                    <div class="me-3">
                    ...
                    <div>
                        <h4 class="text-dark fw-bold mb-2">
                            Dear <span
                                class="text-uppercase"><?php echo ucfirst($_SESSION["logger_username1"]); ?></span>,
                        </h4>
                        <p class="text-dark"
                            style="font-size: 1.1rem; font-style: italic; font-weight: 600; color: #cb1629 !important;">
                            "Your attendance percentage is currently less than the required percentage
                            (<strong><?php echo $attendance; ?>%</strong>).
                            <br>Your current attendance: <strong><?php echo $allot_attendance; ?>%</strong>
                            <br>
                            <span class="fw-bold">Please contact your <b>HOD</b> for further assistance."</span>
                        </p>
                    </div>
                </div>
            </div>
                                ...
                                <div class="row">
                                    <?php
                                        ...
                                        if (isset($sem_row['form_start_date']) && $today >= $sem_row['form_start_date']) {
                                            if (isset($status_row['exam_status']) && $status_row["exam_status"] == 'Approve') {

                                        ?>
                                    <div class="card card-secondary" style="width: -webkit-fill-available;">
                                        <div class="card-header">
                                            <h3 class="card-title">INSTRUCTIONS FOR FILLING UP THE EXAMINATION
                                                FORM:
                                            </h3>
                                        </div>
                                        <div class="card-body table-responsive p-0">
                                            <div class="card-body">
                                                <ul>
                                                    <li>Write correct subject code(s) as indicated in your
                                                        Programme
                                                        Guide / Syllabus.</li>
                                                    ...
                                                    <li>It is advised to enclose photocopy of Admit Card / Mark
                                                        sheet /
                                                    ...
                                                    <li>The enrolment number, subject code, papers' name are
                                                        ...
                                                        form.</li>
                                                    <li> Examination fees once paid, not refundable / adjusted
                                                        in any
                                                        case.</li>
    ...
    // $check = "SELECT `fee_id`, `course_id`, `fee_academic_year`, `fee_particulars`, `fee_amount`, `fee_fine`, `fee_start_date`, `fee_lastdate`, `applicable`, `fee_astatus`, `fee_time`, `status` FROM `tbl_fee` WHERE `fee_academic_year` = '$academic_yearId' AND `course_id` = '$course_id' AND `status` = '$visible' AND `fee_particulars` LIKE '%1st installment%'";
    // $result_check = $con->query($check);
    // if ($result_check->num_rows > 0) {
    //     echo "<pre>";
    //     while ($fee_row = $result_check->fetch_assoc()) {
    //         print_r($fee_row);
    //     }
    //     echo "</pre>";
    // } else {
    //     echo "<p>No fee details found for the given academic year and 1st installment.</p>";
    // }
?>
                                    ...
                                    <div class="alert alert-danger"
                                        ...
                                        <ul style="margin: 15px 0; padding-left: 25px; color: #fff;">
                                            <li style="margin-bottom: 8px;">Your fee payment is pending,
                                                or</li>
                                            <li>Your payment verification is in process.</li>
                                        </ul>
                                        <p style="margin-top: 15px;">Please <a href="payment_status_exam"
        ...
        <script>
        function updateExamStatus(admission_id) {
            // Create AJAX request
            var xhr = new XMLHttpRequest();
            xhr.open("POST", "update_exam_status.php", true);
            xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

            // Send admission_id to the PHP script
            xhr.send("admission_id=" + admission_id);
    ...
    function fetch_data() {
        ...
        $.ajax({
            url: 'include/view.php?action=fetch_exam_form',
            type: 'POST',
