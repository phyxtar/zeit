...
if (!empty($_POST) && (sizeof($_POST) > 0)) {
    ...
    if ($_SERVER['HTTP_HOST'] == "localhost") {
        ...
        $furl = "http://localhost/nsucms-aws-latest/student/reg_success";
        $surl = $furl;
    } else {
        $amount = $_SESSION['amount'] . ".00";
        $furl = "https://nsucms.in/nsucms/student/reg_success";
        $surl = $furl;
    }

    $firstname = $_SESSION['registration']['candidate_name'];
    $email = $_SESSION['registration']['email_id'];
    $phone_no = $_SESSION['registration']['mobile_no1'];

    $admission_id = $_SESSION['registration']['admission_id'];
    $semester_id = $_SESSION['registration']['semester_id'];
    ...
    if ($apiname === "initiate_payment") {
        $postData = array(
            ...
            "udf6" => "University_Registration_Form_fee",
            "address1" => "aaaa",
            "address2" => "aaaa",
            "city" => "aaaa",
            "state" => "aaaa",
