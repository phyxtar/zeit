...
if (isset($_POST["action"]) && $_POST["action"] == "export_fees_details") {
 ...
 ";
        ...
        $sql = "SELECT * FROM `tbl_admission` WHERE `status` = '$visible'  AND `admission_session` = '$academic_year'  AND `stud_status` = 1 AND `admission_hostel` = 'yes' && hostel_leave_date` = '' ORDER BY `admission_id` ASC";
    else
        $sql = "SELECT * FROM `tbl_admission`
...
ORDER BY `fee_particulars` ASC ";
            ...
            if (!empty($fees_details)) {
                ...
                $s_no++; // Increment the overall student count for visible rows only
            }
        }
    } else {
    ...
    //Set Filename
    $fileName = "Course-And-Year-Wise-Hostel-Dues-Report-" . date('d-m-Y') . ".xlsx";
    //Set BackGround
