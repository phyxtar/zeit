...
?>
    ...
    <table id="example1" class="table table-bordered table-striped display" style="width:100%;">
        ...
        <thead>
            <tr>
                ...
                <th width="10%">Fees Details <span id="total_fee_show">test</span></th>
                    ...
                    <tr>
                        ...
                        <table>
                                ...
                                <tbody>
                                    ...
                                    ?>
                                        ...
                                        <tr>
                                            <td><?= $tmpSNo; ?></td>
                                            <td><?= $arrayTblFeeUpdate->fee_particulars; ?></td>
                                            <td>&#8377; <?= $arrayTblFeeUpdate->fee_amount; ?></td>
                                            <td>&#8377; <?= $arrayTblFeeUpdate->fee_paid; ?></td>
                                            <td> <?= $arrayTblFeeUpdate->payment_date ? '' : date('d-M-Y', strtotime($arrayTblFeeUpdate->payment_date)); ?></td>
                                            <td> <?= date('d-M-Y', strtotime($arrayTblFeeUpdate->fee_last_date)); ?></td>
                                            <td> <?= $arrayTblFeeUpdate->txn_no; ?></td>
                                            <td>&#8377; <?= $arrayTblFeeUpdate->fee_rebate; ?></td>
                                            <td>&#8377; <?= $fine_by_particular_remaning ?></td>
                                            <td>&#8377; <?= $total_remaing_amount_final   ?></td>
                                            <td>
                        ...
                        <td> <?php
                                ...
                                $fee_status = "SELECT *
                                     ...
                                     ";
                                $fee_status_result = $con->query($fee_status);
                                $fee_status_data = $fee_status_result->fetch_assoc();
                                if (mysqli_num_rows($fee_status_result) && $fee_status_data['fee_status'] == "Approve") {
                                    $status = "Approve";
                                }


                                ?>
                            <button id="status<?= $row['admission_id'] ?>" onclick="change_status(<?= $row['admission_id'] ?>,<?= $status ?>)" class="btn btn-<?= $status == "Approve" ? 'success' : 'danger' ?> btn-sm">
                                <i id="status-fa<?= $row['admission_id'] ?>" class="fas fa-user-<?= $status == "Approve" ? 'check' : 'times' ?> "></i>
                            </button>
                        </td>
    ...
    <div class="row" id="total_fee_hide">
        ...
        <div class="col-2">
            <div class="btn btn-primary btn-sm">Total Fee - <?= $total_fee_amount ?></div>
        </div>
        <div class="col-2">
            <div class="btn btn-success btn-sm">Total Fee Paid - <?= $total_paid_amount ?></div>
        </div>
        <div class="col-2">
            <div class="btn btn-warning btn-sm">Total Fee Rebate - <?= $total_rebate_amount ?></div>
        </div>
        <div class="col-2">
            <div class="btn btn-danger btn-sm">Total Due - <?= (int)$total_fee_amount - (int)$total_paid_amount  ?></div>
        </div>
        <div class="col-2">
            <div class="btn btn-secondary btn-sm"> <strong>Overall Paid - <?= (int)$total_rebate_amount + (int)$total_paid_amount  ?> </strong> </div>
        </div>
    </div>
<?php
}
