...
<table id="dtHorizontalExample" class="table table-bordered table-striped">
    ...
    <tbody>
        <?php
            ...
            $sql = "SELECT * FROM `tbl_backlogs` WHERE `status` = '$visible' && `semester_id`='$semester_id' && `course_id`='$course_id' && `fee_academic_year`='$academic_year'
                                ORDER BY `semester_id` ASC
                                ";
            $result = $con->query($sql);
            }else{
                $sql = "SELECT * FROM `tbl_backlogs` WHERE `status` = '$visible' ORDER BY `semester_id` ASC
                ";
            $result = $con->query($sql);
            }
        ...
        <tr>
            ...
            <?php
                        $sql_sem = "SELECT * FROM `tbl_backlogs`
            ...
            <td class="project-actions text-center">
                <button class="btn btn-info btn-sm"
                    onclick="document.getElementById('edit_backlogs_list<?php echo $row["semester_id"]; ?>').style.display='block'">
            ...
            <div id="edit_backlogs_list<?php echo $row["semester_id"]; ?>" class="w3-modal" style="z-index:2020;">
                <div class="w3-modal-content w3-animate-top w3-card-4" style="width:55%">
                    <header class="w3-container" style="background:#343a40; color:white;">
                        <span
                            onclick="document.getElementById('edit_backlogs_list<?php echo $row["semester_id"]; ?>').style.display='none'"
                            class="w3-button w3-display-topright">&times;</span>
                        <h2 align="center">Edit Semester</h2>
                    </header>
                    <form id="edit_backlogs_list_form<?php echo $row["semester_id"]; ?>" role="form" method="POST">
                        <div class="card-body">
                            ...
                            <input type='hidden' name='action' id="action<?php echo $row["semester_id"]; ?>"
                                value='edit_backlogs_list' />
                            <div class="col-md-12" id="edit_loader_section<?php echo $row["semester_id"]; ?>"></div>
                            <button type="button" id="edit_backlogs_list_button<?php echo $row["semester_id"]; ?>"
                    ...
                    $(function() {
                        ...
                        $('#edit_backlogs_list_button<?php echo $row["semester_id"]; ?>').click(function() {
                            ...
                            $('#edit_backlogs_list_button<?php echo $row["semester_id"]; ?>').prop(
                                'disabled', true);
                                ...
                                success: function(result) {
                                    ...
                                    if (result == "success") {
                                        ...
                                        function showUpdatedData() {
                                            $.ajax({
                                                url: 'include/view.php?action=get_backlog',
                                                type: 'POST',
                                    ...
                                    $('#edit_backlogs_list_button<?php echo $row["semester_id"]; ?>')
                                        .prop('disabled', false);
                                }
