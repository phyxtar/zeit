<?php
if ($_GET["action"] == "get_loan_application") {
?>
    <table id="leave_application_table" class="table table-bordered table-striped">
        ...
        <tbody>
            <?php
            $sql = "SELECT * FROM `tbl_loan` ORDER BY `id` ASC";
            ...
            ?>
                    <tr>
                        ...
                        if ((isset($permissions['2']) && in_array('2_9', explode('||', $permissions['2']))) || $loggedInUserType == 'admin') {
                            ?>
                             <button type="button"
                        ...
                        if ((isset($permissions['2']) && in_array('2_11', explode('||', $permissions['2']))) || $loggedInUserType == 'admin') {
                            ?>
                        ...
                        if ((isset($permissions['2']) && in_array('2_10', explode('||', $permissions['2']))) || $loggedInUserType == 'admin') {
                            ?>
                            <button class="btn btn-danger btn-sm" onclick="document.getElementById('delete_loan_app<?php echo $row["id"]; ?>').style.display='block'">
                        ...
                        ?>
                            ...
                            <!-- Loan delete Section Start -->
                            <div id="delete_loan_app<?php echo $row["id"]; ?>" class="w3-modal" style="z-index:2020;">
                                <div class="w3-modal-content w3-animate-top w3-card-4" style="width:40%">
                                    <header class="w3-container" style="background:#343a40; color:white;">
                                        <span onclick="document.getElementById('delete_loan_app<?php echo $row["id"]; ?>').style.display='none'" class="w3-button w3-display-topright">&times;</span>
                                        <h2 align="center">Are you sure???</h2>
                                    </header>
                                    <form id="delete_loan_app_form<?php echo $row["id"]; ?>" role="form" method="POST">
                                        <div class="card-body">
                                            <div class="col-md-12" id="delete_error_section<?php echo $row["id"]; ?>"></div>
                                            <div class="col-md-12" align="center">
                                                <input type='hidden' name='delete_loan_app_id' id="delete_loan_app_id<?php echo $row["id"]; ?>" value='<?php echo $row["id"]; ?>' />
                                                <input type='hidden' name='action' id="action_delete<?php echo $row["id"]; ?>" value='delete_loan_applications' />
                                                <div class="col-md-12" id="delete_loader_section<?php echo $row["id"]; ?>"></div>
                                                <button type="button" id="delete_loan_app_button<?php echo $row["id"]; ?>" class="btn btn-danger">Move To Trash</button>
                                                <button type="button" onclick="document.getElementById('delete_loan_app<?php echo $row["id"]; ?>').style.display='none'" class="btn btn-primary">Cancel</button>
                                            </div>
                                        </div>
                                    </form>
                                    <script>
                                        $(function() {
                                            $('#delete_loan_app_button<?php echo $row["id"]; ?>').click(function() {
                                                ...
                                                $('#delete_loan_app_button<?php echo $row["id"]; ?>').prop('disabled', true);
                                                var action = $("#action_delete<?php echo $row["id"]; ?>").val();
                                                var delete_loan_app_id = $("#delete_loan_app_id<?php echo $row["id"]; ?>").val();
                                                var dataString = 'action=' + action + '&delete_loan_app_id=' + delete_loan_app_id;

                                                $.ajax({
                                                    url: 'loan_applications_delete.php',
                                                    type: 'POST',
                                                    data: dataString,
                                                    success: function(result) {
                                                        ...
                                                        if (result == "success") {
                                                            ...
                                                            function showDeletedData() {
                                                                $.ajax({
                                                                    url: 'include/view.php?action=get_loan_application',
                                                                    type: 'GET',
                                                                    success: function(result) {
                            ...
                            <div id="add_rejection_reason" class="w3-modal" style="z-index:2020;">
                                <div class="w3-modal-content w3-animate-top w3-card-4" style="width:40%">
                                    ...
                                    <form id="rejectionForm" action="loan_rejection_reason.php" method="POST">
                                        <div class="card-body">
    ...
    <script>
        ...
        $(document).ready(function() {
            $('form#rejectionForm').on('submit', function(event) {
                ...
                console.log('Submitting rejection for teacher ID:', teacherId);

                $.ajax({
                    url: 'loan_rejection_reason.php',
                    type: 'POST',
                    data: form.serialize(),
                    dataType: 'json',
                    success: function(response) {
                        if (response.status === 'success') {
                            console.log('Rejection successful for teacher ID:', teacherId);
                            window.location.reload();
                        } else {
                            console.log('Rejection failed:', response.message);
            ...
            $('#leave_application_table').on('click', '.reject-btn', function() {
                ...
                var teacher_id = button.data('id');
                $.ajax({
                    url: 'update_loan_reject_btn.php',
                    type: 'POST',
                    data: {
                        teacher_id: teacher_id
                    },
                    ...
                    success: function(response) {
                        if (response === 'yes') {
                            button.text('Rejected').removeClass('btn-danger').addClass('btn-secondary');
                        } else if (response === 'why_rejected is empty') {
                            button.text('Reject').removeClass('btn-secondary').addClass('btn-danger');
                        } else {
                            alert('Error updating reject button');
                        }
                    }
                });
            });
